@include('frontend.partials.pages-portion.supports.support-01')
                    ...
                    <li>
                        <div class="btn-wrapper">
                            @if(Auth::guard('web')->check() || Auth::guard('admin')->check())
                                ...
                                <a class="btn-default btn-outline register" href="{{$route}}"
                                   style="padding: 5px 25px 5px;font-size: 15px">{{__('Dashboard')}}</a>
                            @else
                                @if(!empty(get_static_option('register_show_hide')))
                                    <a class="btn-default btn-outline register" href="{{route('user.register')}}"
