...
class RecentBlogWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $blog = Blog::where(['status' => 'publish'])->orderBy('views', 'DESC')->take($post_items)->get();
        ...
        foreach ($blog as $post) {
            ...
            $category_route = route('frontend.blog.category.single', ['id' => optional($post->category)->id]);
            ...
            $output .= '<div class="recent-contents">
                        ...
                        <div class="content">
                            ...
                            <h4 class="common-title"><a href="' . route('frontend.blog.single', $post->slug) . '">' . purify_html(Str::words($post->getTranslation('title', $user_selected_language), 6)) . '</a></h4>
                        </div>
                    </div>';
        }
        $output .= '</div>';
