...
class UserDashboardController extends Controller
{
    ...
    public function user_index(){
        ...
        $total_post = Product::where('user_id',Auth::guard('web')->user()->id)->count();
        $all_auth_user_data = Product::where('user_id',Auth::guard('web')->user()->id)->orderBy('id','desc')->where('status','publish')->take(5)->get();
    ...
    {
        $user_details = User::find(Auth::guard('web')->user()->id);
        $all_wishlist = UserWishlist::where('user_id', $user_details->id)->paginate(10);

        return view('frontend.user.dashboard.user-wishlist')->with(
            [
                'user_details' => $user_details,
    ...
    public function store_user_wishlist($slug)
    {
        $product = Product::where('slug', $slug)->select('id')->firstOrFail();
        $wishlist_query = UserWishlist::query();
        $wishlist_old = $wishlist_query->where(['user_id' => Auth::guard('web')->user()->id, 'product_id' => $product->id])->first();

        if ($wishlist_old == null)
        {
            $wishlist_query->insert([
               'user_id'=> Auth::guard('web')->user()->id,
               'product_id'=> $product->id
            ]);
        } else {
            $wishlist_query->where(['user_id' => Auth::guard('web')->user()->id, 'product_id' => $product->id])->delete();

            return back()->with(['msg' => __('Product Removed from Wishlist'), 'type' => 'danger']);
        }

        return back()->with(['msg' => __('Product Wishlisted Successfully'), 'type' => 'success']);
    }
