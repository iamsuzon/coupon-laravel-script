...
class AboutSectionStyleOne extends PageBuilderBase
{
    ...
    {
        return 'about-section/abouts-01.png';
    }
    ...
    public function frontend_render(): string
    {
        ...
        $data['description'] = Str::words(strip_tags($this->setting_item('description_' . $current_lang), 55));

        $data['button_text'] = SanitizeInput::esc_html($this->setting_item('button_text_'. $current_lang) ?? '');
        $data['button_url'] = SanitizeInput::kses_basic($this->setting_item('button_url') ?? '');
        $data['bg_image'] = render_background_image_markup_by_attachment_id($this->setting_item('image'));

        return self::view_render('about.about_top', $data);
    }

    public function addon_title()
    {
        return __('About Area');
    }
