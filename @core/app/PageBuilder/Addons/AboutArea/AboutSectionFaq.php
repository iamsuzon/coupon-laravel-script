...
class AboutSectionFaq extends PageBuilderBase
{
    use LanguageFallbackForPageBuilder, RenderFrontendView;

    public function preview_image()
    {
        return 'about-section/about-05.png';
    }
    ...
    public function frontend_render(): string
    {
        ...
        $data['pagination_status'] = $this->setting_item('pagination_status')?? '';

        $faq_items = Faq::query()->orderBy($order_by,$order);
        if(!empty($items)){
            $data['faqs'] = $faq_items->paginate($items);
        }else{
            $data['faqs'] =  $faq_items->get();
        }

        return self::view_render('about.faq_section', $data);
    }

    public function addon_title()
    {
        return __('About Area Faq');
    }
