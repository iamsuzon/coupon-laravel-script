...
class HomePagePopularOffer extends PageBuilderBase
{
    use LanguageFallbackForPageBuilder, RenderFrontendView;

    public function preview_image()
    {
        return 'home/home-01/home-1-today-popular-offer.png';
    }
    ...
    public function frontend_render()
    {
        ...
        $data['limit_product'] = $this->setting_item('limit_product') ?? null;

        $all_deals = Product::with('brand','location','category','reviews')->where(['status' => 'publish']);
        ...
        if(!empty($data['limit_product'])){
            $data['deals'] = $all_deals->orderBy('views', $order)->take($data['limit_product'])->get();
        }else{
            $data['deals'] = $all_deals->orderBy('views', $order)->take(8)->get();
        }

        $data['padding_top'] = $this->setting_item('padding_top') ?? '';
        $data['padding_bottom'] = $this->setting_item('padding_bottom') ?? '';

        return self::view_render('home.home_page_popular_offer', $data);
    }


    public function addon_title()
    {
        return __('Home Page Popular Offer: 01');
    }
}