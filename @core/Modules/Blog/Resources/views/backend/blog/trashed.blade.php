                                ...
                                @foreach($trashed_blogs as $data)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <span class="{{$colors[random_int(0,4)]}}">{{$data->category->getTranslation('title',$default_lang,true)}}</span>
                                        </td>
                                        <td>{{$data->author}}</td>
                                        <td>
                                        ...
                                        <td>
                                            @can('blog-trashed-delete')
                                                <x-delete-popover-all-lang
                                                        :url="route('admin.blog.trashed.delete',$data->id)"/>
                                            @endcan

                                            @can('blog-trashed-restore')
                                                <a class="btn btn-lg btn-primary btn-sm mb-3 mr-1"
                                                   href="{{route('admin.blog.trashed.restore',$data->id)}}">
                                                    {{__('Restore')}}
                                                </a>
                                            @endcan
